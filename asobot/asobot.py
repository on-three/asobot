...
DATE: Sunday, Sept 14th 2014

Pass IRC commands to an on-stream game emulator
...
DEFAULT_PORT = 6660
LOG_FILENAME = 'Asobot.log'
LOG_DIRECTORY = '/.Asobot'
...
class Asobot(twisted_irc.IRCClient):
  plugins = []
...
class AsobotFactory(protocol.ClientFactory):
  protocol = Asobot
...
def main():
  ...
  #always log to daily file in ~/.Asobot unlesss we have verbose option
  if args.verbose:
  ...
  Asobot.nickname = credentials['nickname']
  Asobot.realname = credentials['realname']
  Asobot.username = credentials['username']
  Asobot.password = credentials['password']
  ...
  factory = AsobotFactory(hostname, network)
  if args.ssl:
